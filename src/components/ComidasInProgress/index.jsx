...
function FoodsInProgress({ data: recipe }) {
  ...
  useEffect(() => {
    const updateChecked = () => {
      ...
      if (inProgressRecipes) {
        ...
        if (meal) {
          const newArr = [];
          meal.forEach((number) => newArr.push(recipe[`strIngredient${number}`]));
          setChecked(newArr);
        }
      } else {
  ...
  useEffect(() => {
    const updateLocalStorage = () => {
      const inProgressRecipes = JSON.parse(localStorage.getItem('inProgressRecipes'));
      const newArr = [];
      const IngredientsWithNumber = getIngredientsWithNumber(recipe);
      checkedIngredients.forEach((element) => newArr
        .push(IngredientsWithNumber[element]));
      localStorage.setItem('inProgressRecipes', JSON.stringify({
        ...inProgressRecipes,
        meals: { ...inProgressRecipes.meals, [id]: newArr },
      }));
    };
    updateLocalStorage();
