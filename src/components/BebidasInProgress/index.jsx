...
function DrinksInProgress({ data }) {
  ...
  useEffect(() => {
    const updateChecked = () => {
      ...
      if (inProgressRecipes) {
        ...
        if (cocktail) {
          const newArr = [];
          cocktail.forEach((number) => newArr.push(data[`strIngredient${number}`]));
          setChecked(newArr);
        }
      } else {
        const obj = {
  ...
  useEffect(() => {
    const updateLocalStorage = () => {
      const inProgressRecipes = JSON.parse(localStorage.getItem('inProgressRecipes'));
      const newArr = [];
      const IngredientsWithNumber = getIngredientsWithNumber(data);
      checkedIngredients.forEach((element) => newArr
        .push(IngredientsWithNumber[element]));
      localStorage.setItem('inProgressRecipes', JSON.stringify({
        ...inProgressRecipes,
        cocktails: { ...inProgressRecipes.cocktails, [id]: newArr },
      }));
    };
    updateLocalStorage();
